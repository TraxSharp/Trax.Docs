...
- For **one-off jobs** that should run once and auto-disable, use [ScheduleOnceAsync](/docs/sdk-reference/scheduler-api/manifest-management#scheduleonceasync) instead of `ScheduleAsync`. It creates a manifest with `ScheduleType.Once` — no `Schedule` object needed. See [Delayed / One-Off Jobs](/docs/scheduler/delayed-jobs) for details.
