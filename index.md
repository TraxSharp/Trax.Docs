...
Trax is a .NET framework for structuring business logic as typed pipelines. Each pipeline is a sequence of steps where every step has one job, typed input and output, and errors short-circuit automatically. Your configuration lives in `Program.cs`, your logic lives in pipelines.
...
| **Junction** | A point on the route where work happens (`Step<TIn, TOut>`). The train either continues right (success) or switches left (failure) |
