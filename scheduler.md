...
A working example with the built-in PostgreSQL task server, bulk scheduling, metadata cleanup, and the dashboard is in [`samples/Trax.Samples.Scheduler.Hangfire`](https://github.com/TraxSharp/Trax.Core/tree/main/samples/Trax.Samples.Scheduler.Hangfire).
